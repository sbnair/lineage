...
namespace QuantConnect.Lean.Engine.TransactionHandlers
{
    ...
    public class BrokerageTransactionHandler : ITransactionHandler
    {
        ...
        /// <summary>
        /// The OpenPositionOrders holds all orders wich corresponds to opened position.
        /// </summary>
        private OpenPositionOrders openPositionOrders;
        ...
        public virtual void Initialize(IAlgorithm algorithm, IBrokerage brokerage, IResultHandler resultHandler)
        {
            ...
            openPositionOrders = new OpenPositionOrders(_algorithm.LiveMode);
        ...
        /// <summary>
        /// Cancel outstanding TP/SL/expiry when order is filled
        /// </summary>
        /// <param name="order">Filled order</param>
        private void HandleOCO(Order order)
        {
            // remove order expiry if any
            _algorithm.ScheduledEventCollection.Remove(order.Tag);

            // cancel SL/TP if any
            var orders_to_cancel = GetOpenOrdersByTag(order.Tag);

            if (orders_to_cancel != null)
            {
                foreach (var order_to_cancel in orders_to_cancel)
                {
                    _algorithm.Transactions.CancelOrder(order_to_cancel.Id, order_to_cancel.Tag);
                }
            }

            // remove order from the position list
            var open_order = openPositionOrders.Values.FirstOrDefault(x => x.Tag == order.Tag);
            if (open_order != null)
            {
                openPositionOrders.Remove(open_order);
            }
        }

        /// <summary>
        /// Update submitted position close limit orders when order expires.
        /// </summary>
        /// <param name="order">Filled expiry order</param>
        private void HandleExpiry(Order order)
        {
            // cancel limit orders to close position after expiry
            var expired_quantity = Math.Abs(order.Quantity);
            foreach (var _order in Orders.Values.Where(x => (x.Status == OrderStatus.Submitted || x.Status == OrderStatus.UpdateSubmitted) && 
                                                             x.Reason == OrderReason.Close && x.Type == OrderType.Limit && x.Symbol == order.Symbol))
            {
                expired_quantity -= Math.Abs(_order.Quantity);
                    
                if (expired_quantity < 0)
                {
                    // update limit order quantity
                    UpdateOrder(new UpdateOrderRequest(order.Time,
                                                        order.Id, 
                                                        new UpdateOrderFields { Quantity = Math.Abs(expired_quantity) * Math.Sign(_order.Quantity) }));

                    break;
                }
                else // expired_quantity >= 0
                {
                    _algorithm.Transactions.CancelOrder(_order.Id, _order.Tag);
                    if(expired_quantity == 0) { break; }
                }
            }

            // close remaining TP/SL if any
            HandleOCO(order);           
        }

        /// <summary>
        /// Handles partial or full position close
        /// </summary>
        /// <param name="order">Filled close order</param>
        private void HandlePositionClosed(Order order)
        {
            var orderQuantity = Math.Abs(order.Quantity);
            var open_position_orders = openPositionOrders.Values.Where(x => (x.Symbol == order.Symbol));
            if (!open_position_orders.IsNullOrEmpty())
            {
                foreach (var openPositionOrder in open_position_orders)
                {
                    orderQuantity -= Math.Abs(openPositionOrder.Quantity);

                    if (orderQuantity < 0)
                    {
                        // update expiry order size
                        if(openPositionOrder.ExpiryDate != null)
                        {
                            ScheduledEvent _event = _algorithm.ScheduledEventCollection.GetScheduledEventByName(openPositionOrder.Tag);
                            if (_event != null)
                            {
                                _algorithm.ScheduledEventCollection.Remove(_event.Name);
                                ScheduledEventParams scheduledEventParams = new ScheduledEventParams(openPositionOrder.Tag,
                                                                                                    openPositionOrder.ExpiryDate.Value, 
                                                                                                    openPositionOrder.Symbol, 
                                                                                                    (-1) * orderQuantity * Math.Sign(openPositionOrder.Quantity),
                                                                                                    (OrderReason)openPositionOrder.Reason);
                                _algorithm.ScheduledEventCollection.Add(scheduledEventParams);
                            }
                        }

                        // update order size in the position list
                        openPositionOrder.Quantity = (-1) * orderQuantity * Math.Sign(openPositionOrder.Quantity);
                        openPositionOrder.LastUpdateTime = order.Time;

                        // update submitted TP/SL sizes
                        var update_orders = GetOpenOrdersByTag(openPositionOrder.Tag);
                        foreach (var update_order in update_orders)
                        {
                            UpdateOrder(new UpdateOrderRequest(order.Time, update_order.Id, 
                                                new UpdateOrderFields { Quantity =  (-1) * orderQuantity * Math.Sign(update_order.Quantity)}));
                        }
        ...
        /// <summary>
        /// Handles the position when oder is filled
        /// </summary>
        /// <param name="orderEvent">Order event</param>
        private void HandleOrderFilled(OrderEvent orderEvent)
        {   
            ...
            if (order != null && order.Status == OrderStatus.Filled)
            {
                switch (order.Reason)
                {
                    case OrderReason.Expiry:
                        HandleExpiry(order);
                        break;

                    case OrderReason.TakeProfit:
                    case OrderReason.StopLoss:
                        HandleOCO(order);
                        break;

                    case OrderReason.Close:
                        HandlePositionClosed(order);
                        break;

                    case OrderReason.Open:
                        openPositionOrders.Add(order);
                        if(order.ExpiryDate != null)
                        {
                            ScheduledEventParams scheduledEventParams = new ScheduledEventParams(order.Tag,
        ...
        /// <summary>
        /// Get the order by its tag
        /// </summary>
        /// <param name="tag">Order tag to fetch</param>
        /// <returns>A clone of the order with the specified tag, or null if no match is found</returns>
        public Order GetOrderByTag(string tag)
        {
            var order = _openOrders.FirstOrDefault(x => x.Value.Tag.Contains(tag)).Value
                        ?? _completeOrders.FirstOrDefault(x => x.Value.Tag.Contains(tag)).Value;
            return order?.Clone();
        }


        /// <summary>
        /// Get the open order by its tag
        /// </summary>
        /// <param name="tag">Order tag to fetch</param>
        /// <returns>A clone of the order with the specified tag, or null if no match is found</returns>
        public Order GetOpenOrderByTag(string tag)
        {
            var order = _openOrders.FirstOrDefault(x => x.Value.Tag.Contains(tag)).Value;
            return order?.Clone();
        }
        ...
        /// <summary>
        /// Get the list of open orders by its tag
        /// </summary>
        /// <param name="tag">Order tag to fetch</param>
        /// <returns>A clone of the orders with the specified tag, or null if no match is found</returns>
        public List<Order> GetOpenOrdersByTag(string tag)
        {
            var orders = _openOrders.Select(x => x.Value).Where(x => x.Tag.Contains(tag));
            if (orders != null)
            {
                return orders.Select(x => x.Clone()).ToList();
            }
            else
            {
                return null;
                ...
                switch (orderEvent.Status)
                {
                    ...
                    case OrderStatus.Filled:
                        ...
                        if (orderEvent.Status == OrderStatus.Filled && !string.IsNullOrWhiteSpace(orderEvent.Message))
                        {
                            ...
                            {
                                //TODO: add orderEvent.Message to Log (?) create separate property for Message in Order (?)
                                // order.Tag += " - " + orderEvent.Message;
                            }
                        }
                        break;
                ...
                case OrderType.Limit:
                    {
                        ...
                        if (increment > 0)
                        {
                            ...
                            // warnings are commented to prevent printing too much in console
