...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        /// <seealso cref="MarketOrder(Symbol, decimal, bool, TimeSpan?, string)"/>
        public OrderTicket Order(Symbol symbol, decimal quantity, bool asynchronous = false, string tag = "")
        {
            return MarketOrder(symbol, quantity, asynchronous, tag: tag);
        }

        /// <summary>
        /// OpenPosition implementation: Send a market order with SL and TP orders and wait for fills.
        /// </summary>
        /// <param name="symbol">Symbol of the MarketType Required.</param>
        /// <param name="quantity">Number of shares to request.</param>
        /// <param name="stopLoss">Number of pips from current price to stop loss.</param>
        /// <param name="takeProfit">Number of pips from current price to take profit.</param>
        /// <param name="expiryTime">Timespan till the expiry of the order</param>
        /// <param name="asynchronous">Send the order asynchrously (false). Otherwise we'll block until it fills</param>
        public void OpenPosition(Symbol symbol, int quantity, double stopLoss = 0.0, double takeProfit = 0.0, TimeSpan? expiryTime = null, bool asynchronous = false)
        {
            string tag = OrderTicket.GenerateTag();
            var security = Securities[symbol];
            decimal current_price = security.Close;

            int side = Math.Sign(quantity);
            decimal stop_loss_price;
            decimal take_profit_price;

            stop_loss_price = current_price - (decimal)stopLoss * side;
            take_profit_price = current_price + (decimal)takeProfit * side;
            
            MarketOrder(symbol, (decimal)quantity, asynchronous, expiryTime, tag, OrderReason.Open);
            if (take_profit_price != current_price)
            {
                LimitOrder(symbol, quantity * (-1), take_profit_price, tag: tag, orderReason: OrderReason.TakeProfit);
            }
            if (stop_loss_price != current_price)
            {
                StopMarketOrder(symbol, quantity * (-1), stop_loss_price, tag);
            }

        }

        /// <summary>
        /// OpenPositionLimit implementation: Send a limit order and wait for it to be filled.
        /// </summary>
        /// <param name="symbol">Symbol of the MarketType Required.</param>
        /// <param name="quantity">Number of shares to request.</param>
        ...
        /// <param name="expiryTime">Timespan till the expiry of the order</param>
        /// <returns>int Order id</returns>
        public OrderTicket OpenPositionLimit(Symbol symbol, decimal quantity, double limitPrice, TimeSpan? expiryTime = null)
        {
            return LimitOrder(symbol, quantity, (decimal)limitPrice, expiryTime, orderReason: OrderReason.Open);
        }

        /// <summary>
        /// OpenPositionMarket implementation: Send a market order and wait for it to be filled.
        /// </summary>
        /// <param name="symbol">Symbol of the MarketType Required.</param>
        /// <param name="quantity">Number of shares to request.</param>
        /// <param name="asynchronous">Send the order asynchrously (false). Otherwise we'll block until it fills.</param>
        /// <param name="expiryTime">Timespan till the expiry of the order</param>
        /// <returns>int Order id</returns>
        public OrderTicket OpenPositionMarket(Symbol symbol, decimal quantity, TimeSpan? expiryTime = null, bool asynchronous = false)
        {
            return MarketOrder(symbol, quantity, asynchronous, expiryTime, orderReason: OrderReason.Open);
        }

        /// <summary>
        /// ClosePosition implementation: Send an order to close position.
        /// </summary>
        /// <param name="symbol">Symbol of the MarketType Required.</param>
        /// <param name="quantity">Number of shares to request.</param>
        /// <param name="orderType">orderType limit or market.</param>
        /// <param name="limitPrice">limitPrice for limit order to close position</param>
        /// <returns>int Order id</returns>
        public OrderTicket ClosePosition(Symbol symbol, int quantity, OrderType orderType, double limitPrice = 0.0)
        {
            if (orderType == OrderType.Limit)
            {
                if (limitPrice != 0.0)
                {
                    return LimitOrder(symbol, (decimal)quantity, (decimal)limitPrice, orderReason: OrderReason.Close);
                }
                else
                {
                    throw new Exception($"QCAlgorithm.ClosePosition() -> For LimitOrder limitPrice is equal to {limitPrice}");
                }
                
            }
            else if (orderType == OrderType.Market)
            {
                return MarketOrder(symbol, (decimal)quantity, orderReason: OrderReason.Close);
            }

            else
            {
                throw new Exception($"QCAlgorithm.ClosePosition() is implemented only for {OrderType.Limit.ToString()} and {OrderType.Market.ToString()}");
            }
        }
        ...
        /// <returns>int Order id</returns>
        public OrderTicket MarketOrder(Symbol symbol, int quantity, bool asynchronous = false, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return MarketOrder(symbol, (decimal)quantity, asynchronous, expiryTime, tag, orderReason: orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>int Order id</returns>
        public OrderTicket MarketOrder(Symbol symbol, double quantity, bool asynchronous = false, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return MarketOrder(symbol, (decimal)quantity, asynchronous, expiryTime, tag, orderReason: orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>int Order id</returns>
        public OrderTicket MarketOrder(Symbol symbol, decimal quantity, bool asynchronous = false, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            DateTime? expiryDate;
            expiryDate = expiryTime != null ? this.UtcTime + expiryTime : null;
            ...
            {
                var mooTicket = MarketOnOpenOrder(security.Symbol, quantity, expiryTime, tag, orderReason);
            ...
            var request = CreateSubmitOrderRequest(OrderType.Market, security, quantity, tag, DefaultOrderProperties?.Clone(), expiryDate: expiryDate, orderReason: orderReason);
        ...
        public OrderTicket MarketOnOpenOrder(Symbol symbol, double quantity, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return MarketOnOpenOrder(symbol, (decimal)quantity, expiryTime, tag, orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>The order ID</returns>
        public OrderTicket MarketOnOpenOrder(Symbol symbol, int quantity, TimeSpan? expiryTime, string tag = "", OrderReason? orderReason = null)
        {
            return MarketOnOpenOrder(symbol, (decimal)quantity, expiryTime, tag, orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>The order ID</returns>
        public OrderTicket MarketOnOpenOrder(Symbol symbol, decimal quantity, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            var security = Securities[symbol];
            var request = CreateSubmitOrderRequest(OrderType.MarketOnOpen, security, quantity, tag, DefaultOrderProperties?.Clone(), expiryDate: this.UtcTime + expiryTime, orderReason: orderReason);
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>The order ID</returns>
        public OrderTicket MarketOnCloseOrder(Symbol symbol, int quantity, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return MarketOnCloseOrder(symbol, (decimal)quantity, expiryTime, tag, orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>The order ID</returns>
        public OrderTicket MarketOnCloseOrder(Symbol symbol, double quantity, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return MarketOnCloseOrder(symbol, (decimal)quantity, expiryTime, tag, orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>The order ID</returns>
        public OrderTicket MarketOnCloseOrder(Symbol symbol, decimal quantity, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            var security = Securities[symbol];
            var request = CreateSubmitOrderRequest(OrderType.MarketOnClose, security, quantity, tag, DefaultOrderProperties?.Clone(), expiryDate: this.UtcTime + expiryTime, orderReason: orderReason);
            var response = PreOrderChecks(request);
            if (response.IsError)
            {
                return OrderTicket.InvalidSubmitRequest(Transactions, request, response);
        ...
        public OrderTicket LimitOrder(Symbol symbol, int quantity, decimal limitPrice, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return LimitOrder(symbol, (decimal)quantity, limitPrice, expiryTime, tag, orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>Order id</returns>
        public OrderTicket LimitOrder(Symbol symbol, double quantity, decimal limitPrice, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            return LimitOrder(symbol, (decimal)quantity, limitPrice, expiryTime, tag, orderReason);
        }
        ...
        /// <param name="orderReason">The reason for this order.</param>
        /// <returns>Order id</returns>
        public OrderTicket LimitOrder(Symbol symbol, decimal quantity, decimal limitPrice, TimeSpan? expiryTime = null, string tag = "", OrderReason? orderReason = null)
        {
            DateTime? expiryDate;
            expiryDate = expiryTime != null ? this.UtcTime + expiryTime : null;

            var security = Securities[symbol];
            var request = CreateSubmitOrderRequest(OrderType.Limit, security, quantity, tag, limitPrice: limitPrice, properties: DefaultOrderProperties?.Clone(), expiryDate: expiryDate, orderReason: orderReason);
            var response = PreOrderChecks(request);
            if (response.IsError)
            {
                return OrderTicket.InvalidSubmitRequest(Transactions, request, response);
        ...
        {
            var security = Securities[symbol];
            var request = CreateSubmitOrderRequest(OrderType.StopMarket, security, quantity, tag, stopPrice: stopPrice, properties: DefaultOrderProperties?.Clone(), orderReason:  OrderReason.StopLoss);
            var response = PreOrderChecks(request);
            if (response.IsError)
            {
                return OrderTicket.InvalidSubmitRequest(Transactions, request, response);
        ...
        /// <seealso cref="MarketOrder(QuantConnect.Symbol,decimal,bool,TimeSpan?,string)"/>
        ...
        private void SetHoldingsImpl(Symbol symbol, decimal orderQuantity, bool liquidateExistingHoldings = false, string tag = "")
        {
            ...
            if (Math.Abs(quantity) > 0)
            {
                ...
                if (security.Exchange.ExchangeOpen)
                {
                    MarketOrder(symbol, quantity, false, tag: tag);
                }
                else
                {
                    MarketOnOpenOrder(symbol, quantity, tag: tag);
                }
            }
        }
        ...
        private SubmitOrderRequest CreateSubmitOrderRequest(OrderType orderType, Security security, decimal quantity, string tag, IOrderProperties properties, decimal stopPrice = 0m, decimal limitPrice = 0m, 
            DateTime? expiryDate = null, OrderReason? orderReason = null)
        {
            return new SubmitOrderRequest(orderType, security.Type, security.Symbol, quantity, stopPrice, limitPrice, UtcTime, tag, properties, expiryDate, orderReason);
        }
    }
}
