...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        /// <summary>
        /// Creates a new Differential indicator.
        /// </summary>
        /// <param name="symbol">The symbol whose DIFF we want</param>
        /// <param name="resolution">The resolution.</param>
        /// <param name="selector">Selects a value from the BaseData to send into the indicator, if null defaults to the Value property of BaseData (x => x.Value)</param>
        ...
        /// <returns>The Differential indicator for the requested symbol over the specified period</returns>
        public Differential DIFF(Symbol symbol, int window_size, Func<DateTime, CalendarInfo> time_frame_func, SubscriptionManager subs_manager,  Resolution? resolution = null, Func<IBaseData, IBaseDataBar> selector = null)
        {
            var name = CreateIndicatorName(symbol, $"DIFF({window_size})", resolution);
            var differential = new Differential(name, window_size, time_frame_func, subs_manager, symbol);
            RegisterIndicator(symbol, differential, resolution, selector);

            if (EnableAutomaticIndicatorWarmUp)
            {
                WarmUpIndicator(symbol, differential, resolution);
            }
        ...
        /// <param name="subs_manager">SubcscriptionManaget of Algorithm</param>
        /// <returns>The Differential indicator for the requested symbol over the specified period</returns>
        public Differential DIFF(Symbol symbol, int window_size, int time_frame, SubscriptionManager subs_manager, Resolution? resolution = null, Func<IBaseData, IBaseDataBar> selector = null)
        {
            var name = CreateIndicatorName(symbol, $"DIFF({window_size})", resolution);
            var differential = new Differential(name, window_size, time_frame, subs_manager, symbol);
            RegisterIndicator(symbol, differential, resolution, selector);
        ...
        /// <param name="subs_manager">SubcscriptionManaget of Algorithm</param>
        /// <returns>The Differential indicator for the requested symbol over the specified period</returns>
        public FakeNetwork FN(Symbol symbol, int window_size, int h, int time_frame, SubscriptionManager subs_manager, Resolution? resolution = null, Func<IBaseData, IBaseDataBar> selector = null)
        {
            var name = CreateIndicatorName(symbol, $"FN({window_size})", resolution);
            var fake_network= new FakeNetwork(name, window_size, h, time_frame, subs_manager, symbol);
            RegisterIndicator(symbol, fake_network, resolution, selector);

            if (EnableAutomaticIndicatorWarmUp)
            {
                WarmUpIndicator(symbol, fake_network, resolution);
            }
        ...
        /// <summary>
        /// Creates a new ABSReturn indicator.
        /// </summary>
        /// <param name="symbol">The symbol whose abs return we seek</param>
        /// <param name="period">The period of the abs return.</param>
        /// <param name="resolution">The resolution.</param>
        /// <param name="selector">Selects a value from the BaseData to send into the indicator, if null defaults to casting the input value to a TradeBar.</param>
        /// <returns>abs return indicator for the requested symbol.</returns>
        public ABSReturn ABSR(Symbol symbol, int period, Resolution? resolution = null, Func<IBaseData, decimal> selector = null)
        {
            var name = CreateIndicatorName(symbol, $"ABSR({period})", resolution);
            var absReturn = new ABSReturn(name, period);
            RegisterIndicator(symbol, absReturn, resolution, selector);

            if (EnableAutomaticIndicatorWarmUp)
            {
                WarmUpIndicator(symbol, absReturn, resolution);
            }
