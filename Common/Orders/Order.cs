...
namespace QuantConnect.Orders
{
    ...
    public abstract class Order
    {
        ...
        /// <summary>
        /// Gets the utc expiry date of the order, or null if was not specified.
        /// </summary>
        [JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
        public DateTime? ExpiryDate { get; internal set; }

        /// <summary>
        /// Gets the reason for the order, or null if was not specified.
        /// </summary>
        [JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
        public OrderReason? Reason { get; internal set; }
        ...
        public static Order CreateOrder(SubmitOrderRequest request)
        {
            return CreateOrder(request.OrderId, request.OrderType, request.Symbol, request.Quantity, request.Time,
                request.Tag, request.OrderProperties, request.LimitPrice, request.StopPrice, request.ExpiryDate, request.Reason);
        }

        private static Order CreateOrder(int orderId, OrderType type, Symbol symbol, decimal quantity, DateTime time,
            string tag, IOrderProperties properties, decimal limitPrice, decimal stopPrice, DateTime? expiryDate, OrderReason? orderReason)
        {
            Order order;
            switch (type)
            {
                case OrderType.Market:
                    order = new MarketOrder(symbol, quantity, time, expiryDate, tag, orderReason, properties);
                    break;

                case OrderType.Limit:
                    order = new LimitOrder(symbol, quantity, limitPrice, time, expiryDate, tag, orderReason, properties);
                    break;

                case OrderType.StopMarket:
                    order = new StopMarketOrder(symbol, quantity, stopPrice, time, expiryDate, tag, properties);
                    break;

                case OrderType.StopLimit:
                    order = new StopLimitOrder(symbol, quantity, stopPrice, limitPrice, time, expiryDate, tag, properties);
                    break;

                case OrderType.MarketOnOpen:
                    order = new MarketOnOpenOrder(symbol, quantity, time, expiryDate, tag, orderReason, properties);
                    break;

                case OrderType.MarketOnClose:
                    order = new MarketOnCloseOrder(symbol, quantity, time, expiryDate, tag, orderReason, properties);
                    break;
